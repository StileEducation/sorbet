    ...
    BasicBlock *cont;
    {
        ...
        for (auto &argExpr : md.args) {
            ...
            if (auto *opt = ast::cast_tree<ast::OptionalArg>(argExpr)) {
                ...
                synthesizeExpr(presentCont, present, core::LocOffsets::none(), make_unique<ArgPresent>(md.symbol, i));
                ...
                if (info.type) {
                    ...
                    synthesizeExpr(defaultNext, tmp, defLoc, make_unique<Cast>(result, info.type, core::Names::let()));
                    result = tmp;
                }
